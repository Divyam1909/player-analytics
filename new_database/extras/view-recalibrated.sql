-- ============================================
-- FIX: RECALIBRATED PERFORMANCE INDICES
-- Drop and recreate with better formulas
-- ============================================
...
SELECT 
  ...
  -- AWAY TEAM BASIC STATS (same structure)
  ...
  -- ===========================================
  -- RECALIBRATED PERFORMANCE INDICES (HOME)
  -- Using percentage-based formulas for differentiation
  -- ===========================================
  
  -- PCI: Pass Completion % weighted with Progressive Pass ratio
  CASE 
    WHEN COUNT(CASE WHEN pe.team_id = m.home_team_id THEN 1 END) > 0 THEN
      LEAST(100, GREATEST(0, ROUND(
        (COUNT(CASE WHEN pe.team_id = m.home_team_id AND pe.is_successful = true THEN 1 END)::NUMERIC * 100 / 
         NULLIF(COUNT(CASE WHEN pe.team_id = m.home_team_id THEN 1 END), 0)) * 0.7 +
        (COUNT(CASE WHEN pe.team_id = m.home_team_id AND pe.is_progressive_pass = true THEN 1 END)::NUMERIC * 100 / 
         NULLIF(COUNT(CASE WHEN pe.team_id = m.home_team_id THEN 1 END), 0)) * 0.3
      )))::INTEGER
    ELSE 50
  END AS home_possession_control_index,
  
  -- CCI: Key passes + Assists + Chances, scaled for typical match (10-15 key passes max)
  LEAST(100, GREATEST(0, ROUND(
    (COUNT(CASE WHEN pe.team_id = m.home_team_id AND pe.is_key_pass = true THEN 1 END)::NUMERIC * 100 / NULLIF(15, 0)) * 0.4 +
    (COUNT(CASE WHEN pe.team_id = m.home_team_id AND pe.is_assist = true THEN 1 END)::NUMERIC * 100 / NULLIF(5, 0)) * 0.3 +
    (COUNT(CASE WHEN ftc.team_id = m.home_team_id AND ftc.is_in_box = true THEN 1 END)::NUMERIC * 100 / NULLIF(10, 0)) * 0.3
  )))::INTEGER AS home_chance_creation_index,
  
  -- SE: Goals / Shots ratio (direct conversion rate)
  CASE 
    WHEN COUNT(CASE WHEN sot.team_id = m.home_team_id THEN 1 END) > 0 THEN
      LEAST(100, ROUND(COUNT(CASE WHEN sot.team_id = m.home_team_id AND sot.is_goal = true THEN 1 END)::NUMERIC / 
       COUNT(CASE WHEN sot.team_id = m.home_team_id THEN 1 END) * 100))::INTEGER
    ELSE 0
  END AS home_shooting_efficiency,
  
  -- DS: Based on saves and low goals conceded (inverse relationship with goals)
  LEAST(100, GREATEST(0, 
    50 + 
    COUNT(CASE WHEN ka.team_id = m.home_team_id AND ka.action_type = 'save' THEN 1 END) * 5 -
    COUNT(CASE WHEN ka.team_id = m.home_team_id AND ka.action_type = 'goal_conceded' THEN 1 END) * 15
  ))::INTEGER AS home_defensive_solidity,
  
  -- T&P: Progressive actions relative to total actions
  CASE 
    WHEN COUNT(CASE WHEN pe.team_id = m.home_team_id THEN 1 END) + COUNT(CASE WHEN d.team_id = m.home_team_id THEN 1 END) > 0 THEN
      LEAST(100, GREATEST(0, ROUND(
        ...
        NULLIF(COUNT(CASE WHEN pe.team_id = m.home_team_id THEN 1 END) + COUNT(CASE WHEN d.team_id = m.home_team_id THEN 1 END), 0) * 2.5
      )))::INTEGER
    ELSE 40
  END AS home_transition_progression,
  
  -- RPE: Aerial duels won % + base recovery factor (25 base, aerials add up to 50)
  25 + LEAST(75, 
    CASE 
      WHEN COUNT(CASE WHEN d.team_id = m.home_team_id AND d.duel_type = 'aerial' THEN 1 END) > 0 THEN
        ROUND(COUNT(CASE WHEN d.team_id = m.home_team_id AND d.duel_type = 'aerial' AND d.is_successful = true THEN 1 END)::NUMERIC * 100 /
         COUNT(CASE WHEN d.team_id = m.home_team_id AND d.duel_type = 'aerial' THEN 1 END) * 0.75)::INTEGER
      ELSE 25
    END
  ) AS home_recovery_pressing_efficiency,

  -- ===========================================
  -- RECALIBRATED PERFORMANCE INDICES (AWAY)
  -- ===========================================
    ...
    WHEN COUNT(CASE WHEN pe.team_id = m.away_team_id THEN 1 END) > 0 THEN
      LEAST(100, GREATEST(0, ROUND(
        (COUNT(CASE WHEN pe.team_id = m.away_team_id AND pe.is_successful = true THEN 1 END)::NUMERIC * 100 / 
         NULLIF(COUNT(CASE WHEN pe.team_id = m.away_team_id THEN 1 END), 0)) * 0.7 +
        (COUNT(CASE WHEN pe.team_id = m.away_team_id AND pe.is_progressive_pass = true THEN 1 END)::NUMERIC * 100 / 
         NULLIF(COUNT(CASE WHEN pe.team_id = m.away_team_id THEN 1 END), 0)) * 0.3
      )))::INTEGER
    ELSE 50
  END AS away_possession_control_index,
  
  LEAST(100, GREATEST(0, ROUND(
    (COUNT(CASE WHEN pe.team_id = m.away_team_id AND pe.is_key_pass = true THEN 1 END)::NUMERIC * 100 / NULLIF(15, 0)) * 0.4 +
    (COUNT(CASE WHEN pe.team_id = m.away_team_id AND pe.is_assist = true THEN 1 END)::NUMERIC * 100 / NULLIF(5, 0)) * 0.3 +
    (COUNT(CASE WHEN ftc.team_id = m.away_team_id AND ftc.is_in_box = true THEN 1 END)::NUMERIC * 100 / NULLIF(10, 0)) * 0.3
  )))::INTEGER AS away_chance_creation_index,
  
  CASE 
    WHEN COUNT(CASE WHEN sot.team_id = m.away_team_id THEN 1 END) > 0 THEN
      LEAST(100, ROUND(COUNT(CASE WHEN sot.team_id = m.away_team_id AND sot.is_goal = true THEN 1 END)::NUMERIC / 
  ...
  LEAST(100, GREATEST(0, 
    50 + 
    COUNT(CASE WHEN ka.team_id = m.away_team_id AND ka.action_type = 'save' THEN 1 END) * 5 -
    COUNT(CASE WHEN ka.team_id = m.away_team_id AND ka.action_type = 'goal_conceded' THEN 1 END) * 15
  ))::INTEGER AS away_defensive_solidity,
  
  CASE 
    WHEN COUNT(CASE WHEN pe.team_id = m.away_team_id THEN 1 END) + COUNT(CASE WHEN d.team_id = m.away_team_id THEN 1 END) > 0 THEN
      LEAST(100, GREATEST(0, ROUND(
        ...
        NULLIF(COUNT(CASE WHEN pe.team_id = m.away_team_id THEN 1 END) + COUNT(CASE WHEN d.team_id = m.away_team_id THEN 1 END), 0) * 2.5
      )))::INTEGER
    ELSE 40
    ...
    CASE 
      WHEN COUNT(CASE WHEN d.team_id = m.away_team_id AND d.duel_type = 'aerial' THEN 1 END) > 0 THEN
        ROUND(COUNT(CASE WHEN d.team_id = m.away_team_id AND d.duel_type = 'aerial' AND d.is_successful = true THEN 1 END)::NUMERIC * 100 /
         COUNT(CASE WHEN d.team_id = m.away_team_id AND d.duel_type = 'aerial' THEN 1 END) * 0.75)::INTEGER
      ELSE 25
    END
...
SELECT 
    ...
    away_defensive_solidity as away_ds,
    away_transition_progression as away_tp,
    away_recovery_pressing_efficiency as away_rpe
FROM match_statistics_summary;

SELECT 'View recalibrated with balanced formulas!' as status;
